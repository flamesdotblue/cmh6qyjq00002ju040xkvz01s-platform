...
const rawProjects = [
  {
    ...
    image: gradientThumb('from-cyan-500 via-teal-400 to-blue-500'),
    link: '#',
  },
  {
    id: 'brand',
    ...
    image: gradientThumb('from-cyan-400 via-blue-500 to-indigo-500'),
    link: '#',
  },
  {
    id: 'social',
    ...
    image: gradientThumb('from-teal-400 via-cyan-500 to-blue-600'),
    link: '#',
  },
  {
    id: 'mobile',
    ...
    image: gradientThumb('from-sky-500 via-cyan-500 to-teal-400'),
    link: '#',
  },
];
        ...
        <div className="relative flex gap-2 rounded-2xl border border-white/15 bg-white/10 p-1 backdrop-blur-xl">
          {tabs.map((t) => (
            ...
            >
              {t}
              {active === t && (
                <motion.span layoutId="tab" className="pointer-events-none absolute inset-0 -z-0 rounded-xl bg-cyan-400/20" />
              )}
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
        {projects.map((p, i) => (
          <motion.button
            ...
            className="group relative overflow-hidden rounded-3xl border border-white/15 bg-white/10 text-left backdrop-blur-xl"
          >
        ...
        {selected && (
          ...
          >
            <motion.div
              role="dialog"
              aria-modal="true"
              className="relative w-full max-w-2xl overflow-hidden rounded-3xl border border-white/15 bg-[#051327]/95 p-0 text-slate-100 shadow-[0_30px_100px_rgba(0,0,0,0.45)] backdrop-blur-2xl"
            ...
            >
              ...
              <button
                ...
                className="absolute right-3 top-3 rounded-xl border border-white/15 bg-white/10 px-3 py-1 text-sm text-slate-100 backdrop-blur-xl hover:bg-white/20 focus:outline-none focus:ring-2 focus:ring-cyan-400/60"
              >
                Close
              </button>
            </motion.div>
...
function gradientThumb(gradient) {
  return gradient;
}
